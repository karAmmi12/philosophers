...
/*   Updated: 2024/06/07 15:48:21 by kammi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_msg(t_philo *philo, int msg)
{
	...
	if (msg == DEAD)
	{
		printf("%s%09zu %zu died%s\n", RED, get_time() - philo->table->start_time, philo->id, RESET);
		set_death(philo->table);
	}
	else if (msg == EAT)
		printf("%s%09zu %zu is eating%s\n",BLUE, get_time() - philo->table->start_time, philo->id, RESET );
	else if (msg == SLEEP)
		printf("%s%09zu %zu is sleeping%s\n", YELLOW,  get_time() - philo->table->start_time, philo->id, RESET);
	else if (msg == THINK)
		printf("%s%09zu %zu is thinking%s\n", GREEN, get_time() - philo->table->start_time, philo->id, RESET);
	else if (msg == FORK)
		printf("%s%09zu %zu has taken a fork%s\n", MAGENTA, get_time() - philo->table->start_time, philo->id, RESET);
	pthread_mutex_unlock(&philo->table->m_print);
}
